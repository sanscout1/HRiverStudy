...
2. 외래키도 기본키 요소르 쓰일 수 있다.
3. 따라서 기본키요소가 2개 이상이라면, 거기에 포함되 있는 요소는 null 값이허용 된다.
4. 시험에서 commit 과 rollback 은 TCL 인것을 인지하자
5. Where 문은 별명에 쓰면 오류난다. 하지만 order by 는 별명에 써도 된다.
6. union 은 select 한 요소가 같아야 가능하다.
