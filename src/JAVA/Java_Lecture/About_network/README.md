...
### JSON
- JSON은 특히 구조화된 데이터를 다룰 때 유용합니다. 예를 들어, 데이터베이스에서 검색한 결과, 사용자 입력, API 응답 등과 같이 구조화된 정보를 표현하는 데에 적합합니다.
- JSON은 이진 데이터(binary data)나 복잡한 연산을 필요로 하는 데이터에는 적합하지 않을 수 있습니다. 예를 들어, 이미지, 음악 파일, 비디오 파일과 같은 이진 데이터는 JSON으로 표현하는 것이 적절하지 않습니다. 또한, 복잡한 수학적 연산이 필요한 데이터의 경우에도 JSON 형식이 적합하지 않을 수 있습니다.
