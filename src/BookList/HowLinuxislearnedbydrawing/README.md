...
- 수정이 없는 부분은 공유 메모리로 서로 공유한다.


### execve() : deamand paging
- fork() 와 동일하게 처음 이 함수를 호출하면 테이블이 존재하지만 물리 주소는 할당 되어 있지 않다.
- 페이지 폴트 처리 될 때마다 페이지 테이블에 물리 주소를 연결한다.
...
- 따라서 서로 공유할 데이터는 공유메모리 공간을 생성해서 사용해야 한다.

### 시그널
- 기초적인 통신방법,  복잡한 작업에는 좋지 않다.
...
## 배타적 제어
- 공유 메모리 때와 다르게 동시에 데이터에 영향을 주는 경우에는 제어가 필요하다.
- 리눅스에서는 같은 프로세스에서 fork 한 경우는 서로 영향을 cow 로 피하지만, 다른 두 프로그램(프로세스)이 제 3 프로그램의 데이터를 조작시 에는 `상호 배제`가 필요 할 수 있따.

- `크리티컬 섹션` : 동시에 실행되면 안되는 처리 흐름
- `아토믹 처리` : 하나의 처리로 다루어야 하는 처리 흐름

- 실행 프로그램에 `flock` 명령어를 이용하여 상호 배제 상태로 실행 하게 할 수 있다.

## 멀티프로세스 와 멀티 스레드

![img.png](picture/multiprocessthread.png)

### 멀티프로세스
- fork() 함수로 실행

### 멀티 스레드
- 페이지 테이블 복사가 필요 없어서 생성 시간 짧음
- 모든 스레드가 자원을 공유해서 자원 소비량 적음
- 메모리 공유해서 협조 동작 좋음
...
- cpu가 사용자 모드로 전환되고 프로세스가 디바이스 드라이버로 부터 처리 완료를 확인해서 결과 받음
...
## 디바이스 파일면 변경 문제
- 다른 저장 장치 추가나, 위치변경, 고장등 장치의 디바이스 파일이 바뀌는 경우가 존재
- systemd의 udev 프로그램이 영구장치명을 장치에 설정 가능
- 단순 마운트용으로는 mount 명령어에 UUID 를  지정하여 구분 가능
