...
#### Comparator는 두 매개변수 객체를 비교하는 것이고, Comparable은 자기 자신과 매개변수 객체를 비교
  - 익명객체생성이 가능하다
    - 익명객체란 이름이 정의 되지 않았다... 상속받아서 정의 하지 않고 쓰는 객체란 뜻
    - interface 같은것을 바로 오버라이딩해서 1회성을 사용하는 것
    - comparable은 this 와 비교하기 때문에 익명객체생성이 불가능하다.
...
- 경계 와일드카드는 선언한 부분에 한정해서만 제약 부여
 ...
 - `SimpleList<String> filterStringValues = SimpleList.function1(intValues);`
 - 반환타입에 제한이 안되있기 때문에 선언할때는 extends Number 에 맞게 만들지만 반환할때는 값이 integer 여도 <String> simplelist 에 저장이 가능하다고 뜸
   - 하지만 runtime 오류 발생
...
- Collection.sort 는 Collection 내부 구조 sort 용 
